/** @type {import('next-sitemap').IConfig} */
module.exports = {
  siteUrl: process.env.SITE_URL || "https://petborder.com",
  generateRobotsTxt: true,
  generateIndexSitemap: false,

  // Exclude all non-public routes
  exclude: [
    "/admin",
    "/admin/*",
    "/dashboard",
    "/dashboard/*",
    "/agency-portal",
    "/agency-portal/*",
    "/vet-portal",
    "/vet-portal/*",
    "/wl",
    "/wl/*",
    "/login",
    "/signup",
    "/auth/*",
    // Exclude all API routes EXCEPT the public docs page
    "/api/admin",
    "/api/admin/*",
    "/api/agencies",
    "/api/agencies/*",
    "/api/agency-leads",
    "/api/agency-leads/*",
    "/api/agency",
    "/api/agency/*",
    "/api/api-keys",
    "/api/api-keys/*",
    "/api/checkout",
    "/api/contact",
    "/api/countries",
    "/api/cron",
    "/api/cron/*",
    "/api/generate-timeline",
    "/api/generate-outbound-timeline",
    "/api/labs",
    "/api/mickleham-status",
    "/api/pdf",
    "/api/pdf/*",
    "/api/pets",
    "/api/pets/*",
    "/api/subscription",
    "/api/subscription/*",
    "/api/timelines",
    "/api/timelines/*",
    "/api/v1",
    "/api/v1/*",
    "/api/vet",
    "/api/vet/*",
    "/api/vet-clinics",
    "/api/vets",
    "/api/webhook",
    "/api/webhook/*",
    "/api/og",
  ],

  // Explicitly add pages that next-sitemap doesn't auto-discover with Next.js 16 App Router
  additionalPaths: async (config) => [
    await config.transform(config, "/guides"),
    await config.transform(config, "/guides/bringing-dog-to-australia"),
    await config.transform(config, "/guides/bringing-cat-australia"),
    await config.transform(config, "/guides/rnatt-test-australia"),
    await config.transform(config, "/guides/pet-import-cost-australia"),
    await config.transform(config, "/guides/australia-pet-quarantine"),
    await config.transform(config, "/guides/moving-to-australia-from-uk-with-pet"),
    await config.transform(config, "/guides/moving-australia-usa-pet"),
    await config.transform(config, "/guides/pet-travel-australia-faq"),
    await config.transform(config, "/tools/breed-checker"),
    await config.transform(config, "/privacy"),
    await config.transform(config, "/terms"),
  ],

  robotsTxtOptions: {
    policies: [
      {
        userAgent: "*",
        allow: "/",
        disallow: [
          "/admin",
          "/dashboard",
          "/agency-portal",
          "/vet-portal",
          "/wl",
          "/auth",
          "/api/",
        ],
      },
    ],
    additionalSitemaps: [],
  },

  // Priority and change frequency per page
  transform: async (config, path) => {
    const priorities = {
      "/": 1.0,
      "/generate": 0.9,
      "/outbound": 0.9,
      "/faq": 0.8,
      "/guides": 0.9,
      "/guides/bringing-dog-to-australia": 0.85,
      "/guides/bringing-cat-australia": 0.85,
      "/guides/rnatt-test-australia": 0.85,
      "/guides/pet-import-cost-australia": 0.85,
      "/guides/australia-pet-quarantine": 0.85,
      "/guides/moving-to-australia-from-uk-with-pet": 0.85,
      "/guides/moving-australia-usa-pet": 0.85,
      "/guides/pet-travel-australia-faq": 0.85,
      "/tools/breed-checker": 0.85,
      "/about": 0.7,
      "/contact": 0.6,
      "/api/docs": 0.6,
      "/privacy": 0.4,
      "/terms": 0.4,
    };

    const changeFrequencies = {
      "/": "weekly",
      "/generate": "monthly",
      "/outbound": "monthly",
      "/faq": "monthly",
      "/guides": "monthly",
      "/guides/bringing-dog-to-australia": "monthly",
      "/guides/bringing-cat-australia": "monthly",
      "/guides/rnatt-test-australia": "monthly",
      "/guides/pet-import-cost-australia": "monthly",
      "/guides/australia-pet-quarantine": "monthly",
      "/guides/moving-to-australia-from-uk-with-pet": "monthly",
      "/guides/moving-australia-usa-pet": "monthly",
      "/guides/pet-travel-australia-faq": "monthly",
      "/about": "monthly",
      "/contact": "yearly",
      "/api/docs": "weekly",
      "/privacy": "yearly",
      "/terms": "yearly",
    };

    return {
      loc: path,
      changefreq: changeFrequencies[path] ?? "monthly",
      priority: priorities[path] ?? 0.5,
      lastmod: new Date().toISOString(),
    };
  },
};
